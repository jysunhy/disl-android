...
#java -Ddisl.classes="./DiSLClass.class" -Xmx1g -cp $DISL_HOME/cobertura.jar:$DISL_HOME/asm-debug-all-4.0.jar:$DISL_HOME/dislserver-unspec.jar:$DISL_HOME/log4j-1.2.9.jar net.sourceforge.cobertura.instrument.Main $DEST/DEX/classes.jar
#jar uf $DEST/DEX/classes.jar com/

echo "Generating .dex from .jar"
echo "hhhhh"
#$D2J_HOME/d2j-jar2dex-core.sh -o $DEST/CLASSES/classes.dex $DEST/DEX/classes.jar
/home/sunh/tdroid/out/host/linux-x86/bin/dx --dex --core-library --no-strict --output=$DEST/CLASSES/classes.dex $DEST/DEX/classes.jar 
#java -jar /home/sunh/disl-android/lib/smali-2.0b5.jar  -a 16  $DEST/DEX/classes.jar  -o $DEST/CLASSES/classes.dex
...
echo "Repacking..."
/home/sunh/tdroid/out/host/linux-x86/bin/aapt -f -I $ANDROID_HOME/platforms/android-17/android.jar -F $DEST/REPACK/MyApp.unsigned.apk  $DEST/CLASSES
...
$ANDROID_HOME/tools/zipalign -v -f 4 $DEST/REPACK/MyApp.unsigned.apk $DEST/REPACK/MyApp.apk
echo "...."
