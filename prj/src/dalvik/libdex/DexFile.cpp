...
int instrument(const char* in, int i_len,char* out, int &o_len){
    ...
    if( client_socket < 0)
    {
        ALOG(LOG_INFO,"HAIYANG","Create Socket Failed!");
        close(client_socket);
        return -1;
    }
    ...
    {
        ALOG(LOG_INFO,"HAIYANG","Retry Connect To %s!",SERVER_IP);
        //close(client_socket);
        //return -1;
	continue;
    }
 ...
 int length = 0;
    ...
    ALOG(LOG_INFO,"HAIYANG","start sending %d bytes", i_len);
    ...
    ALOG(LOG_INFO,"HAIYANG","start receiving bytes");
