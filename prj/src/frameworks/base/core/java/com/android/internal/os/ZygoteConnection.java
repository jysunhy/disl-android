    ...
    boolean runOnce() throws ZygoteInit.MethodAndArgsCaller {
        ...
        try {
            ...
            pid = Zygote.forkAndSpecialize(parsedArgs.uid, parsedArgs.gid,
                    parsedArgs.gids, parsedArgs.debugFlags, rlimits, parsedArgs.niceName);
        } catch (IOException ex) {
        ...
        try {
            if (pid == 0) {
				...
				// HAIYANG
				/*
				System.out.println("NICENAME:"+parsedArgs.niceName);
				try {
					Class cls;

					cls = Class.forName("ch.usi.dag.dislre.AREDispatch");

					Method staticMethod = cls.getDeclaredMethod("mapPID",String.class);  

					staticMethod.invoke(cls,parsedArgs.niceName);
