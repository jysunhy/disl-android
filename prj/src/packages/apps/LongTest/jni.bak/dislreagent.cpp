...
void exit_critical_section(pthread_mutex_t mutex){
...
static pthread_mutex_t to_buff_lock;
...
static pthread_mutex_t obj_free_lock;
...
static pthread_mutex_t tagging_lock;
...
static void _fill_ot_rec(JNIEnv * jni_env, buffer * cmd_buff,
	...
	ot_rec.obj_to_tag = jni_env->NewGlobalRef(to_send);
...
static void pack_object(JNIEnv * jni_env, buffer * buff, buffer * cmd_buff,
	...
	if(to_send != NULL) {
		_fill_ot_rec(jni_env, cmd_buff, object_type, buff, (jstring)to_send);
	}
...
static jshort next_analysis_id () {
	...
	jshort result = -1;
	enter_critical_section(tagging_lock);
	{
		result = avail_analysis_id++;
	}
	exit_critical_section(tagging_lock);
...
static jshort register_method(
	...
	// get string length
	jsize str_len =
			jni_env->GetStringUTFLength(analysis_method_desc);

	// get string data as utf-8
	const char * str =
			jni_env->GetStringUTFChars(analysis_method_desc, NULL);
...
static jlong next_thread_id () {
	...
	jlong result = -1;
	enter_critical_section(to_buff_lock);
	{
		result = avail_thread_id++;
	}
	exit_critical_section(to_buff_lock);
...
static void ot_pack_string_data(JNIEnv * jni_env, buffer * buff,
	...
	// get string length
	jsize str_len = jni_env->GetStringUTFLength(to_send);

	// get string data as utf-8
	const char * str = jni_env->GetStringUTFChars(to_send, NULL);
	check_error(str == NULL, "Cannot get string from java");
	...
	// release string
	jni_env->ReleaseStringUTFChars(to_send, str);
}
...
static void ot_pack_thread_data(JNIEnv * jni_env, buffer * buff,
	...
	//jvmtiError error = (*self)->GetThreadInfo(self, to_send, &info);
	...
	pack_long(buff, thr_net_ref);
 	...
 	Object* threadObj = self->threadObj;
	bool isDaemon = false;
    if (threadObj != NULL) {
	...
	pack_string_utf8(buff, "tmp", strlen("tmp"));
...
static void update_send_status(jobject to_send, jlong * net_ref) {
	...
	update_net_reference(dvmThreadSelf(), to_send, *net_ref);
}
...
static void ot_pack_aditional_data(JNIEnv * jni_env, jlong * net_ref,
	...
	// String - pack data
	if(jni_env->IsInstanceOf(to_send, STRING_CLASS)) {

		update_send_status(to_send, net_ref);
		ot_pack_string_data(jni_env, new_objs_buff, (jstring)to_send, *net_ref);
	}

	// Thread - pack data
	if(jni_env->IsInstanceOf(to_send, THREAD_CLASS)) {

		update_send_status(to_send, net_ref);
		ot_pack_thread_data(jni_env, new_objs_buff, (jstring)to_send, *net_ref);
	}
}
...
static void ot_tag_record(JNIEnv * jni_env, buffer * buff, size_t buff_pos,
	...
	// get net reference
	jlong net_ref =
			get_net_reference(jni_env, dvmThreadSelf(), new_objs_buff, to_send);
...
static void ot_relese_global_ref(JNIEnv * jni_env, buffer * cmd_buff) {
	...
	while(read < cmd_buff_len) {
		...
		jni_env->DeleteGlobalRef(ot_rec.obj_to_tag);
	}
}
...
static void * objtag_thread_loop(void * obj) {
	...
	JNIEnv *jni_env;
	java_vm->AttachCurrentThreadAsDaemon(
			&jni_env, NULL);

	// one spare buffer for new objects
	buffer * new_obj_buff = (buffer*)malloc(sizeof(buffer));
	...
	THREAD_CLASS = jni_env->FindClass("java/lang/Thread");
...
static void * send_thread_loop(void * obj) {
	...
	while(true){
		ALOG(LOG_INFO, "HAIYANG", "liangdan says in send loop");
		sleep(5);
	}
	// exit when the jvm is terminated and there are no msg to process
...
void JNICALL jvmti_callback_class_file_load_hook(
	...
	enter_critical_section(tagging_lock);
	{
		...
		if(jvm_started) {
			loader_id = get_net_reference(jni_env, self,
...
void JNICALL jvmti_callback_object_free_hook(
		jvmtiEnv *self, jlong tag) {

	enter_critical_section(obj_free_lock);
	{
...
void JNICALL jvmti_callback_vm_init_hook(
		jvmtiEnv *self, JNIEnv* jni_env, jthread thread
) {
#ifdef DEBUG
...
static void send_obj_free_buffer() {
	...
	exit_critical_section(obj_free_lock);
}
...
void JNICALL jvmti_callback_vm_death_hook(
	...
	//jthread thread_obj;
	//jvmtiError error = (*self)->GetCurrentThread(self, &thread_obj);
	//check_jvmti_error(self, error, "Cannot get object of current thread.");
	//GetAllThreads
	//SuspendThread
	//ResumeThread
	//GetThreadState
...
//#endif
	...
	jvmtiError error;
	error = (*self)->AddCapabilities(self, &cap);
	check_jvmti_error(self, error,
	...
	error = (*self)->SetEventCallbacks(
			self, &callbacks, (jint) sizeof(callbacks)
	);
	check_jvmti_error(self, error, "Cannot set callbacks");


	error = (*self)->SetEventNotificationMode(self, JVMTI_ENABLE,
			JVMTI_EVENT_CLASS_FILE_LOAD_HOOK, NULL);
	check_jvmti_error(self, error, "Cannot set class load hook");

	error = (*self)->SetEventNotificationMode(self, JVMTI_ENABLE,
			JVMTI_EVENT_OBJECT_FREE, NULL);
	check_jvmti_error(self, error, "Cannot set object free hook");

	error = (*self)->SetEventNotificationMode(self, JVMTI_ENABLE,
			JVMTI_EVENT_VM_START, NULL);
	check_jvmti_error(self, error, "Cannot set jvm start hook");

	error = (*self)->SetEventNotificationMode(self, JVMTI_ENABLE,
			JVMTI_EVENT_VM_INIT, NULL);
	check_jvmti_error(self, error, "Cannot set jvm init hook");

	error = (*self)->SetEventNotificationMode(self, JVMTI_ENABLE,
			JVMTI_EVENT_VM_DEATH, NULL);
	check_jvmti_error(self, error, "Cannot set jvm death hook");

	error = (*self)->SetEventNotificationMode(self, JVMTI_ENABLE,
			JVMTI_EVENT_THREAD_END, NULL);
	check_jvmti_error(self, error, "Cannot set thread end hook");


	error = (*self)->CreateRawMonitor(self, "object tags",
			&tagging_lock);
	check_jvmti_error(self, error, "Cannot create raw monitor");

	error = (*self)->CreateRawMonitor(self, "buffids",
			&to_buff_lock);
	check_jvmti_error(self, error, "Cannot create raw monitor");

	error = (*self)->CreateRawMonitor(self, "obj free",
			&obj_free_lock);
	check_jvmti_error(self, error, "Cannot create raw monitor");
*/
	// read options (port/hostname)
	parse_agent_options(NULL);

	// init blocking queues
	Thread* self = dvmThreadSelf();
	bq_create(self, &utility_q, BQ_UTILITY, sizeof(process_buffs *));
	bq_create(self, &empty_q, BQ_BUFFERS, sizeof(process_buffs *));
	bq_create(self, &objtag_q, BQ_BUFFERS, sizeof(process_buffs *));
	bq_create(self, &send_q, BQ_BUFFERS + BQ_UTILITY, sizeof(process_buffs *));
	...
	for(i = 0; i < BQ_BUFFERS + BQ_UTILITY; ++i) {
		...
		pb->analysis_buff = (buffer *)malloc(sizeof(buffer));
		// allocate buffer
		buffer_alloc(pb->analysis_buff);

		// allocate space for buffer struct
		pb->command_buff = (buffer *)malloc(sizeof(buffer));
		// allocate buffer
