...
Object* get_object(Thread* self, jobject obj){
	return dvmDecodeIndirectRef(self, obj);
}

// only retrieves object tag data
jlong get_tag(Thread* self, jobject obj){
	return get_object(self,obj)->uuid;
}
/****
...
// forward declaration
jlong get_net_reference(JNIEnv * jni_env, Thread* self,
...
static bool _object_is_class(Thread* self,  jobject obj) {
	return dvmIsClassObject(get_object(self, obj));
}
/****
...
static void set_tag(Thread* self, jobject obj, jlong tag){
	get_object(self, obj)->uuid = tag;
}

// does not increment any counter - just sets the values
static jlong _set_net_reference(Thread * self, jobject obj,
	...
	set_tag(self, obj, net_ref);

	return net_ref;
}

static void _pack_class_info(buffer * buff, jlong class_net_ref,
		const char * class_sig, const char * class_gen, jlong class_loader_net_ref,
...
static jlong _set_net_reference_for_class(JNIEnv * jni_env,
	...
	jlong net_ref = _set_net_reference(self, klass,
	...
	const char * class_sig = ((ClassObject*)get_object(self, klass))->descriptor;
	const char * class_gen = "";
	...
	jlong class_loader_net_ref =
			get_net_reference(jni_env, self, buff, class_loader);

	// resolve super class...
	jclass super_class = jni_env->GetSuperclass(klass);
	// ... + super class id
	jlong super_class_net_ref =
			get_net_reference(jni_env, self, buff, super_class);
	...
	jni_env->PopLocalFrame(NULL);

	return net_ref;
}

static jint _get_class_id_for_class(JNIEnv * jni_env, Thread* self,
		buffer * buff, jclass klass) {

	jlong class_net_ref = get_tag(self, klass);

	if(class_net_ref == 0) {
		class_net_ref =
				_set_net_reference_for_class(jni_env, self, buff, klass);
	}

	return net_ref_get_class_id(class_net_ref);
}

static jint _get_class_id_for_object(JNIEnv * jni_env, Thread* self,
	...
	jint class_id = _get_class_id_for_class(jni_env, self, buff, klass);
...
static jlong _set_net_reference_for_object(JNIEnv * jni_env,
		Thread* self, buffer * buff, jobject obj) {

	// resolve class id
	jint class_id = _get_class_id_for_object(jni_env, self, buff, obj);

	// assign new net reference
	jlong net_ref =
			_set_net_reference(self, obj, avail_object_id, class_id, 0, 0);
...
jlong get_net_reference(JNIEnv * jni_env, Thread* self,
	...
	// access object tag
	jlong net_ref = get_tag(self, obj);

	// set net reference
	if(net_ref == 0) {

		// decide setting method
		if(_object_is_class(self, obj)) {
			// we have class object
			net_ref = _set_net_reference_for_class(jni_env, self,
					new_obj_buff, (jclass)obj);
		}
		else {
			// we have non-class object
			net_ref = _set_net_reference_for_object(jni_env, self,
					new_obj_buff, obj);
		}
	}
...
// !!! invocation of this method should be protected by lock until the reference
// is queued for sending
void update_net_reference(Thread * self, jobject obj, jlong net_ref) {
	set_tag(self,obj,net_ref);
}
