...
public class USBForwarder {
    ...
    public static void combineAnalysis (
        final String analysisServerIP, final int analysisPort, final int localPort) {
        try {
            ...
            while (true) {
                ...
                // if(isAndroid == null){
                // sockToAndroid = new Socket(InetAddress.getByName
                // ("127.0.0.1"), localPort);
                // sockToAndroid.setSoTimeout (10000);
                // isAndroid = new DataInputStream(new
                // BufferedInputStream(sockToAndroid.getInputStream()));
                // }

                final Socket sockToAndroid = new Socket (
                    InetAddress.getByName ("127.0.0.1"), localPort);
                sockToAndroid.setSoTimeout (20000);
                isAndroid = new DataInputStream (new BufferedInputStream (
                    sockToAndroid.getInputStream ()));
                int pid;
                try {
                ...
                final FileOutputStream fos = new FileOutputStream (new File (
                    "debug_events." + pid), true);
                if (!connectionMap.containsKey (pid)) {
                    try {
                        final Socket sockToAnalysis = new Socket (
                            InetAddress.getByName (analysisServerIP), analysisPort);
                        final DataOutputStream osAnalysis = new DataOutputStream (
                            new BufferedOutputStream (
                                sockToAnalysis.getOutputStream ()));
                ...
                try {
                    ...
                    while (readLen < total) {
                        ...
                        // System.out.println
                        // ("Transmitting "+len+" bytes of data from pid "+pid +
                        // "of "+total);
                    }
                } catch (final Exception e) {
                    e.printStackTrace ();
        ...
        try {
            while (true) {
                try {
                    ...
                    final DataOutputStream osAndroid = new DataOutputStream (
                        new BufferedOutputStream (sockToAndroid.getOutputStream ()));
                    final DataInputStream isAndroid = new DataInputStream (
                        new BufferedInputStream (sockToAndroid.getInputStream ()));
    ...
    public static void combineInstrumentation_one (
        final String instrServerIP, final int instrPort, final int localPort) {
        String info = "";
        try {
            final Socket sockToAndroid = new Socket (
                InetAddress.getByName ("127.0.0.1"), localPort);
            ...
            while (true) {
                ...
                final Socket sockToInstr = new Socket (
                    InetAddress.getByName (instrServerIP), instrPort);
                final DataOutputStream osInstr = new DataOutputStream (
                    new BufferedOutputStream (sockToInstr.getOutputStream ()));;
                final DataInputStream isInstr = new DataInputStream (
                    new BufferedInputStream (sockToInstr.getInputStream ()));;
    ...
    public static void combineInstrumentation (
        final String instrServerIP, final int instrPort, final int localPort) {
        String info = "";
        try {
            while (true) {
    ...
    static class InstrThread implements Runnable {
        ...
        public void run () {
            ...
            USBForwarder.combineInstrumentation_one ("127.0.0.1", 6667, 6100);
