...
final class HasNextMonitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<HasNextMonitor> {
	...
	final void event_hasnext(final Iterator i) {
		int numAlive = 0 ;
		for(int i_1 = 0; i_1 < this.size; i_1++){
			final HasNextMonitor monitor = this.elements[i_1];
...
class HasNextMonitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
    ...
    protected Object clone() {
		try {
			final HasNextMonitor ret = (HasNextMonitor) super.clone();
			return ret;
		}
	...
	final boolean Prop_1_event_hasnext(final Iterator i) {
		{
		}

		final int nextstate = this.handleEvent(0, Prop_1_transition_hasnext) ;
	...
	final boolean Prop_1_event_next(final Iterator i) {
		...
		final int nextstate = this.handleEvent(1, Prop_1_transition_next) ;
		this.Prop_1_Category_match = nextstate == 2;
	...
	final void Prop_1_handler_match (){
		{
			...
			AREDispatch.NativeLog ("next called without hasNext!");
		}
...
public class HasNextRuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	...
	public static final void hasnextEvent(final Iterator i) {
		...
		if ((i == HasNext_i_Map_cachekey_i) ) {
			...
			{
				// FindOrCreateEntry
				final MapOfMonitor<HasNextMonitor> itmdMap = HasNext_i_Map;
				matchedLastMap = itmdMap;
				final HasNextMonitor node_i = HasNext_i_Map.getNodeEquivalent(wr_i) ;
				matchedEntry = node_i;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			final HasNextMonitor created = new HasNextMonitor() ;
