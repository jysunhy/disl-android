...
final class SafeSyncMapMonitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<SafeSyncMapMonitor> {
	...
	final void event_sync(final Map syncMap) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			final SafeSyncMapMonitor monitor = this.elements[i];
	...
	final void event_createSet(final Map syncMap, final Set mapSet) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			final SafeSyncMapMonitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
	...
	final void event_syncCreateIter(final Set mapSet, final Iterator iter) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			final SafeSyncMapMonitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
	...
	final void event_asyncCreateIter(final Set mapSet, final Iterator iter) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			final SafeSyncMapMonitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
	...
	final void event_accessIter(final Iterator iter) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			final SafeSyncMapMonitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
...
class SafeSyncMapDisableHolder extends DisableHolder implements ISafeSyncMapMonitor {
	SafeSyncMapDisableHolder(final long tau) {
		super(tau);
	}
...
class SafeSyncMapMonitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractSynchronizedMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject, ISafeSyncMapMonitor {
    ...
    protected Object clone() {
		try {
			final SafeSyncMapMonitor ret = (SafeSyncMapMonitor) super.clone();
			return ret;
		}
	...
	SafeSyncMapMonitor(final long tau, final CachedWeakReference RVMRef_syncMap) {
		this.tau = tau;
		Prop_1_state = 0;
	...
	final boolean Prop_1_event_sync(final Map syncMap) {
		Set mapSet = null;
		if(Ref_mapSet != null){
	...
	final boolean Prop_1_event_createSet(final Map syncMap, final Set mapSet) {
		Iterator iter = null;
		if(Ref_iter != null){
			iter = (Iterator)Ref_iter.get();
		}
	...
	final boolean Prop_1_event_syncCreateIter(final Set mapSet, final Iterator iter) {
		Map syncMap = null;
		if(Ref_syncMap != null){
	...
	final boolean Prop_1_event_asyncCreateIter(final Set mapSet, final Iterator iter) {
		Map syncMap = null;
		if(Ref_syncMap != null){
			syncMap = (Map)Ref_syncMap.get();
		}
	...
	final boolean Prop_1_event_accessIter(final Iterator iter) {
		Map syncMap = null;
		if(Ref_syncMap != null){
			syncMap = (Map)Ref_syncMap.get();
		}
	...
	final void Prop_1_handler_match (){
		{
			...
			AREDispatch.NativeLog("synchronized collection accessed in non threadsafe manner!");
		}
...
public class SafeSyncMapRuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	...
	public static final void syncEvent(final Map syncMap) {
		...
		// D(X) main:1
		final SafeSyncMapMonitor matchedLeaf = matchedEntry.getValue3() ;
		if ((matchedLeaf == null) ) {
			if ((wr_syncMap == null) ) {
				wr_syncMap = new CachedWeakReference(syncMap) ;
			}
			if ((matchedLeaf == null) ) {
				// D(X) main:4
				final SafeSyncMapMonitor created = new SafeSyncMapMonitor(SafeSyncMap_timestamp++, wr_syncMap) ;
				matchedEntry.setValue3(created) ;
				final SafeSyncMapMonitor_Set enclosingSet = matchedEntry.getValue2() ;
				enclosingSet.add(created) ;
			}
			// D(X) main:6
			final SafeSyncMapMonitor disableUpdatedLeaf = matchedEntry.getValue3() ;
			disableUpdatedLeaf.setDisable(SafeSyncMap_timestamp++) ;
		}
		// D(X) main:8--9
		final SafeSyncMapMonitor_Set stateTransitionedSet = matchedEntry.getValue2() ;
	...
	public static final void createSetEvent(final Map syncMap, final Set mapSet) {
		while (!SafeSyncMap_RVMLock.tryLock()) {
			Thread.yield();
		}
		...
		if (SafeSyncMap_activated) {
			...
			if ((matchedLeaf == null) ) {
				...
				}
				{
					...
					{
						// FindCode
						final Tuple3<MapOfAll<MapOfMonitor<ISafeSyncMapMonitor>, SafeSyncMapMonitor_Set, ISafeSyncMapMonitor>, SafeSyncMapMonitor_Set, SafeSyncMapMonitor> node_syncMap = SafeSyncMap_syncMap_mapSet_iter_Map.getNodeEquivalent(wr_syncMap) ;
						if ((node_syncMap != null) ) {
							final SafeSyncMapMonitor itmdLeaf = node_syncMap.getValue3() ;
							sourceLeaf = itmdLeaf;
						}
					}
					if ((sourceLeaf != null) ) {
						...
						if (definable) {
							...
							final Tuple3<MapOfAll<MapOfMonitor<ISafeSyncMapMonitor>, SafeSyncMapMonitor_Set, ISafeSyncMapMonitor>, SafeSyncMapMonitor_Set, SafeSyncMapMonitor> node_syncMap = SafeSyncMap_syncMap_mapSet_iter_Map.getNodeEquivalent(wr_syncMap) ;
							if ((node_syncMap != null) ) {
								final Tuple3<MapOfMonitor<ISafeSyncMapMonitor>, SafeSyncMapMonitor_Set, ISafeSyncMapMonitor> node_syncMap_mapSet = node_syncMap.getValue1() .getNodeEquivalent(wr_mapSet) ;
								if ((node_syncMap_mapSet != null) ) {
									final ISafeSyncMapMonitor itmdLeaf = node_syncMap_mapSet.getValue3() ;
									if ((itmdLeaf != null) ) {
						...
						if (definable) {
							// D(X) defineTo:6
							final SafeSyncMapMonitor created = (SafeSyncMapMonitor)sourceLeaf.clone() ;
							matchedEntry.setValue3(created) ;
							matchedLeaf = created;
							final SafeSyncMapMonitor_Set enclosingSet = matchedEntry.getValue2() ;
							enclosingSet.add(created) ;
							// D(X) defineTo:7 for <syncMap>
							{
								...
								final SafeSyncMapMonitor_Set targetSet = node_syncMap.getValue2() ;
							...
							// D(X) defineTo:7 for <mapSet-syncMap, mapSet>
							{
								...
								final SafeSyncMapMonitor_Set targetSet = node_mapSet.getValue1() ;
								targetSet.add(created) ;
							}
						}
					}
				...
				if ((disableUpdatedLeaf == null) ) {
					final SafeSyncMapDisableHolder holder = new SafeSyncMapDisableHolder(-1) ;
	...
	public static final void syncCreateIterEvent(final Set mapSet, final Iterator iter) {
		while (!SafeSyncMap_RVMLock.tryLock()) {
			Thread.yield();
		}
		...
		if (SafeSyncMap_activated) {
			...
			// D(X) main:1
			final ISafeSyncMapMonitor matchedLeaf = matchedEntry.getValue2() ;
			if ((matchedLeaf == null) ) {
				if ((wr_mapSet == null) ) {
					wr_mapSet = new CachedWeakReference(mapSet) ;
				}
				...
				}
				{
					...
					{
						// FindCode
						final Tuple2<SafeSyncMapMonitor_Set, SafeSyncMapMonitor> node_mapSet = SafeSyncMap_mapSet__To__syncMap_mapSet_Map.getNodeEquivalent(wr_mapSet) ;
						if ((node_mapSet != null) ) {
							final SafeSyncMapMonitor_Set itmdSet = node_mapSet.getValue1() ;
							sourceSet = itmdSet;
						}
					}
					if ((sourceSet != null) ) {
						...
						for (int ielem = 0; (ielem < setlen) ;++ielem) {
							final SafeSyncMapMonitor sourceMonitor = sourceSet.get(ielem) ;
							if ((!sourceMonitor.isTerminated() && (sourceMonitor.RVMRef_syncMap.get() != null) ) ) {
								sourceSet.set(numalive++, sourceMonitor) ;
								final CachedWeakReference wr_syncMap = sourceMonitor.RVMRef_syncMap;
								...
								ISafeSyncMapMonitor destLeaf = null;
								{
									...
									final MapOfMonitor<ISafeSyncMapMonitor> itmdMap = node_syncMap_mapSet.getValue1() ;
									destLastMap = itmdMap;
									final ISafeSyncMapMonitor node_syncMap_mapSet_iter = node_syncMap_mapSet.getValue1() .getNodeEquivalent(wr_iter) ;
								...
								if (((destLeaf == null) || destLeaf instanceof SafeSyncMapDisableHolder) ) {
									...
									if (definable) {
										// FindCode
										final Tuple2<SafeSyncMapMonitor_Set, ISafeSyncMapMonitor> node_iter = SafeSyncMap_iter_Map.getNodeEquivalent(wr_iter) ;
										if ((node_iter != null) ) {
											final ISafeSyncMapMonitor itmdLeaf = node_iter.getValue2() ;
											if ((itmdLeaf != null) ) {
									...
									if (definable) {
										// FindCode
										final MapOfSetMonitor<SafeSyncMapMonitor_Set, ISafeSyncMapMonitor> node_mapSet = SafeSyncMap_mapSet_iter_Map.getNodeEquivalent(wr_mapSet) ;
										if ((node_mapSet != null) ) {
											final Tuple2<SafeSyncMapMonitor_Set, ISafeSyncMapMonitor> node_mapSet_iter = node_mapSet.getNodeEquivalent(wr_iter) ;
											if ((node_mapSet_iter != null) ) {
												final ISafeSyncMapMonitor itmdLeaf = node_mapSet_iter.getValue2() ;
												if ((itmdLeaf != null) ) {
													if (((itmdLeaf.getDisable() > sourceMonitor.getTau() ) || ((itmdLeaf.getTau() > 0) && (itmdLeaf.getTau() < sourceMonitor.getTau() ) ) ) ) {
														definable = false;
													}
									...
									if (definable) {
										// D(X) defineTo:6
										final SafeSyncMapMonitor created = (SafeSyncMapMonitor)sourceMonitor.clone() ;
										...
										// D(X) defineTo:7 for <iter>
										{
											...
											final SafeSyncMapMonitor_Set targetSet = node_iter.getValue1() ;
											targetSet.add(created) ;
										}
										// D(X) defineTo:7 for <mapSet, iter>
										{
											...
											final SafeSyncMapMonitor_Set targetSet = node_mapSet_iter.getValue1() ;
											targetSet.add(created) ;
										}
										// D(X) defineTo:7 for <syncMap>
										{
										...
										// D(X) defineTo:7 for <syncMap, mapSet>
										{
											...
											final SafeSyncMapMonitor_Set targetSet = node_syncMap_mapSet.getValue2() ;
											targetSet.add(created) ;
										}
									}
								}
			...
			// D(X) main:8--9
			final SafeSyncMapMonitor_Set stateTransitionedSet = matchedEntry.getValue1() ;
	...
	public static final void asyncCreateIterEvent(final Set mapSet, final Iterator iter) {
		while (!SafeSyncMap_RVMLock.tryLock()) {
			Thread.yield();
		}
	...
	public static final void accessIterEvent(final Iterator iter) {
		while (!SafeSyncMap_RVMLock.tryLock()) {
			Thread.yield();
		}
