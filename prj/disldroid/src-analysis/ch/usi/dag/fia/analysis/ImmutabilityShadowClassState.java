...
//	---
	 ...
	 * classes.
	 */
	public ImmutabilityShadowClassState(final ShadowClass cls) {
		...
		for (ShadowClass current = cls; current != null; current = current
			...
			for (final FieldInfo field : current.getDeclaredFields()) {
		...
		for (int i = 0; i < fieldsMetaInfo.size(); ++i) {
			...
			// needed
			final FieldMetaInfo fmi = fieldsMetaInfo.get(i);
...
//		---
     ...
     */
    private static String getFieldId(final FieldNode field) {
	...
	public int getFieldOrdinalNumber(final String fieldId) {
