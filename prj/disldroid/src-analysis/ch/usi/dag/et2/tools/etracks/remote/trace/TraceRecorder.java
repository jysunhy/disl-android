...
public final class TraceRecorder {
    ...
    private static final int __DEATH_COUNT_THRESHOLD__ = 10000;
    private static final ObjectDeathRecord
        __DUMMY_DEATH_RECORD__ = new ObjectDeathRecord (0, 0);
    ...
    private final LinkedList <EventRecord> __exitDeaths = Lists.newLinkedList ();

    /**
     * The latest death record added to the trace. Updated when adding new death
     * records.
     */
    private ObjectDeathRecord __latestDeath = __DUMMY_DEATH_RECORD__;
    ...
    synchronized public void recordObjectExitDeath (final long time, final long objectId, final long lastUseTime) {
        __exitDeaths.add (new ExitObjectDeathRecord (time, objectId, lastUseTime));
    }
    ...
    synchronized public void dumpRecords (final File file) {
        ...
        try {
            ...
            final EventRecord [] exitDeaths = __sortList (__exitDeaths);
            __dumpRecords (exitDeaths, out);

            out.close ();

        } catch (final IOException ioe) {
            System.err.printf (
                "error: failed to dump trace to %s: %s\n",
    ...
    private static void __dumpRecords (final EventRecord [] records, final Formatter out) {
