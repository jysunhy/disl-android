...
final class HasNextMonitorDiSL_SetDiSL extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<HasNextMonitorDiSL> {

	HasNextMonitorDiSL_SetDiSL(){
		this.size = 0;
		this.elements = new HasNextMonitorDiSL[4];
	}
	final void event_hasnext(Iterator i) {
		int numAlive = 0 ;
		for(int i_1 = 0; i_1 < this.size; i_1++){
			HasNextMonitorDiSL monitor = this.elements[i_1];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final HasNextMonitorDiSL monitorfinalMonitor = monitor;
...
class HasNextMonitorDiSL extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			HasNextMonitorDiSL ret = (HasNextMonitorDiSL) super.clone();
			return ret;
		}
...
public class HasNextRuntimeMonitorDiSL implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	...
	private static HasNextMonitorDiSL HasNext_i_Map_cachevalue;
	private static final MapOfMonitor<HasNextMonitorDiSL> HasNext_i_Map = new MapOfMonitor<HasNextMonitorDiSL>(0) ;
	...
	public static final void hasnextEvent(Iterator i) {
		...
		MapOfMonitor<HasNextMonitorDiSL> matchedLastMap = null;
		HasNextMonitorDiSL matchedEntry = null;
		...
		if ((i == HasNext_i_Map_cachekey_i) ) {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<HasNextMonitorDiSL> itmdMap = HasNext_i_Map;
				matchedLastMap = itmdMap;
				HasNextMonitorDiSL node_i = HasNext_i_Map.getNodeEquivalent(wr_i) ;
				matchedEntry = node_i;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			HasNextMonitorDiSL created = new HasNextMonitorDiSL() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_i, created) ;
		}
		// D(X) main:8--9
		final HasNextMonitorDiSL matchedEntryfinalMonitor = matchedEntry;
