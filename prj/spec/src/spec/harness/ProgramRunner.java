...
// javac15 -source 1.4 -target 1.4 -cp ../../:/usr/local/lib ProgramRunner.java
//import org.jraf2.cpu.*;
import PCL.PCL;
 ...
 */
	...
	try {
	    ...
	    /*
	    ThreadCPUAccount account = ThreadCPUAccount.getCurrentAccount();
	    BasicCPUAccounting man = (BasicCPUAccounting)account.getManager();
	    account.consumeAllOrNothing();
	    long lastUsage = man.getUsage();
	    */	    


	    /*
	     * PCL-specific management data:
	     */
	    PCL pcl = null;
	    final int mode = pcl.PCL_MODE_USER;
	    int[] events = { pcl.PCL_CYCLES };
	    long[] cycles = new long[1];
	    double[] fp_results = new double[0];
	    long descr = 0;
	    final double GHz = 2.673619;//lglpc26
		...
		System.out.println("problem with PCLquery");
		...
		if (pcl.PCLread(descr, cycles, fp_results, 1) != pcl.PCL_SUCCESS)
		    System.out.println("problem with PCLread");
		...
		long cpuStart = cycles[0];
		if (pcl.PCLread(descr, cycles, fp_results, 1) != pcl.PCL_SUCCESS)
		    System.out.println("problem with PCLread");
		long totalCPU = cycles[0]-cpuStart;
		//System.out.println(className + " CPU user time: " + (((double)totalCPU)/GHz/1000000000));


                Context.out.println( "======= " + className +
		    " official time " + time.toLongString() );
                Context.out.println( "======= " + className +
                    " Finished in " + (((double)(((int)((((double)totalCPU)/GHz/1000000000)*1000))))/1000) + " secs");


		/*
		account.consumeAllOrNothing();
		long finalUsage = man.getUsage();
		long deltaUsage = finalUsage - lastUsage;
		lastUsage = finalUsage;
		System.out.println("\n\n\n");
		System.out.println("JRAF2 usage         = " + deltaUsage);
		*/
